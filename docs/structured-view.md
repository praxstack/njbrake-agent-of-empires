...
With claude-agent-acp 0.52 or newer, AoE skips that one-shot and instead uses the title the agent itself pushes at the end of each turn. The title keeps tracking the agent's summary across turns until you rename the session yourself, at which point it stays put. This still honors `session.smart_rename`; turning the setting off disables both paths.
